	...
	*/
	...
	function Hotellist() {
		...
		if(strstr($pcode, "ADD") !== "") {  
            ...
            //echo $qry4;
			$qryday = "";
			if (isset($row4['p_code']) && $row4['p_code'] !="") {
				$qryday = "";
			} else if (isset($row3['days']) && is_numeric($row3['days'])) {
				$qryday = " AND `day` <> " . (int)$row3['days'];
			}
			$qry1 = "select 	
			...
			while($row1 = mysql_Fetch_assoc($rst1)){
				...
				echo "<div class='col-sm-12'>
						...
						<tbody>
							...
							<td  class='active text-left formHeader'> 
								<input type=number step=0.5 name='roompp[]' class='form-control' placeholder='방갯수'>
							</td>
							<td  class='active text-left formHeader'> 
			...
			// echo $qry2;
			 ...
			 while($row2 = mysql_Fetch_assoc($rst2)){
					...
					echo"<tr>
							...
							<td>{$hinfo['h_name']}</td>
							<td>{$row2['pcnt']}<input type='hidden' name='pcnt[]'  value='".number_format($row2['pcnt'],2)."'></td>
			...
			echo "<div class='col-sm-12'>
						...
						<tbody>
							...
							<td  class='active text-left formHeader'> 
								<input type=number name='roompp[]' class='form-control' step=0.5 placeholder='방갯수'>
							</td>
							<td  class='active text-left formHeader'> 
								<button type=button class='form-control btnadd' placeholder='추가'>추가</button>
							</td>
			...
			$qry2 ="select grand_eCode, sub_eCode, hotel_code,p_code,p_name from hotel_assign
			 ...
			 while($row2 = mysql_Fetch_assoc($rst2)){
					...
					echo"<tr>
							...
							<td>{$hinfo['h_name']}</td>
							<td>{$row2['pcnt']}<input type='hidden' name='pcnt1[]'  value='".number_format($row2['pcnt'],2)."'></td>
	...
	<div id="contentwrapper" class="reservationDetailForm">
		<div class="main_content">
			...
			<form action="<?= $PHP_SELF ?>?division=<?=$division?>&pdx=<?=$pdx?>&sub=<?=$sub?>&st=<?=$st?>&pcode=<?=$pcode?>" name="frmhotel" id="frmhotel" method="post">
				...
				<input type="hidden" name="pname" id="pname" value='<?php echo addslashes($sctour['p_name']);?>'>
