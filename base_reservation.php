...
<?php
	...
	$_resPage    = max(1, intval($_REQUEST['page'] ?? 1));
	$_resPerPage = 50;

	function printProduct() {
		global $dbConn, $division, $pdx, $sub,$st, $user_dbinfo,$tourCategory,$pname,$area1,$area2,$startDate1,$endDate,$ty,$grestimateCode,$estimateCode,$cp,$_resPage,$_resPerPage,$_resPagingHtml;
	   ...
	   //$startDate = "9" - 매일출발;
		...
		$_sql1 = "select a.p_code, a.p_name,a.c_code1,a.c_code2,a.p_type,a.p_own,a.p_day,a.price_0dadult,a.price_4dadult,a.p_week 
		from product_master a where 1=1 && a.p_code not REGEXP 'ADD' && a.p_vend != '0000-00-00' && end_yn!='y'  $qrynm $qrytca $qrycod1 $qrycod2 $qryperiod $startWeek_qry $deptqry group by a.p_code";
	$_sql2 = "select a.p_code, a.p_name,a.c_code1,a.c_code2,a.p_type,a.p_own,a.p_day,a.price_0dadult,a.price_4dadult,a.p_week 
		from product_master a ,product_limit b
		 where 1=1 && a.p_code=b.p_code && b.p_type='R' && a.p_code not REGEXP 'ADD' && end_yn!='y' $qrytca $qrynm $qrycod1 $qrycod2 $qryperiod1 $qryperiod2 $startWeek_qry $deptqry
		 group by a.p_code";

	// 전체 건수
	$_cntRst     = mysql_query("SELECT COUNT(*) as cnt FROM ({$_sql1} UNION {$_sql2}) as _tmp", $dbConn);
	$_totalCount = ($_cntRst) ? (int)mysql_result($_cntRst, 0, 'cnt') : 0;
	$_totalPages  = max(1, (int)ceil($_totalCount / $_resPerPage));
	$_resPage     = min($_resPage, $_totalPages);
	$_offset      = ($_resPage - 1) * $_resPerPage;

	$qry1 = "{$_sql1} UNION {$_sql2} LIMIT {$_offset}, {$_resPerPage}";
	...
	while($row1 = mysql_Fetch_assoc($rst1)){
			...
			$week2   = array("일","월", "화", "수","목","금","토","매일");
					...
					$linkR="base_reservation_m.php?grestimateCode=$grestimateCode&estimateCode=$estimateCode&division=$division&pdx=$pdx&sub=$sub&ty=$ty&pcode={$row1['p_code']}&pricet=1&st=$startDate1&cp=$cp";
					$list .= " <tr>
								...
								<td align='center'>
									<a  href='base_reservation_m.php?grestimateCode=$grestimateCode&estimateCode=$estimateCode&division=$division&pdx=$pdx&sub=$sub&ty=$ty&pcode={$row1['p_code']}&st=$startDate1&pricet=1&cp=$cp' class='btn btn-xs btn-default btnchk'>일반</a>
									<a  href='base_reservation_m.php?grestimateCode=$grestimateCode&estimateCode=$estimateCode&division=$division&pdx=$pdx&sub=$sub&ty=$ty&pcode={$row1['p_code']}&st=$startDate1&pricet=3&cp=$cp' class='btn btn-xs btn-default btnchk'>업체</a>
								</td>
							</tr>"; 
		...
		$_resPagingHtml = '';
		if ($_totalPages > 1) {
			$startPage = max(1, $_resPage - 5);
			$endPage   = min($_totalPages, $_resPage + 4);
			$_resPagingHtml .= "<div class='text-center' style='margin:8px 0;'>";
			$_resPagingHtml .= "<span style='margin-right:10px'>총 {$_totalCount}건 / {$_totalPages}페이지</span>";
			if ($_resPage > 1) {
				$_resPagingHtml .= "<button type='button' class='btn btn-default btn-xs js-respage' data-page='1'>&laquo;</button> ";
				$_resPagingHtml .= "<button type='button' class='btn btn-default btn-xs js-respage' data-page='" . ($_resPage-1) . "'>&lsaquo;</button> ";
			}
			for ($p = $startPage; $p <= $endPage; $p++) {
				$act = ($p == $_resPage) ? " btn-primary" : " btn-default";
				$_resPagingHtml .= "<button type='button' class='btn{$act} btn-xs js-respage' data-page='{$p}'>{$p}</button> ";
			}
			if ($_resPage < $_totalPages) {
				$_resPagingHtml .= "<button type='button' class='btn btn-default btn-xs js-respage' data-page='" . ($_resPage+1) . "'>&rsaquo;</button> ";
				$_resPagingHtml .= "<button type='button' class='btn btn-default btn-xs js-respage' data-page='{$_totalPages}'>&raquo;</button>";
			}
			$_resPagingHtml .= "</div>";
		}
			...
			<div class="row">
				<div class="col-sm-12 col-md-12">
					<form action="<?= $PHP_SELF ?>?grestimateCode=<?=$grestimateCode?>&estimateCode=<?= $estimateCode ?>&division=<?=$division?>&pdx=<?=$pdx?>&sub=<?=$sub?>&ty=<?=$ty?>&cp=<?=$cp?>" enctype="multipart/form-data" name="base_code" id="base_code" method="post" onSubmit='return chksave()'>
						...
						<input type="hidden" name="page" id="resPage" value="<?= $_resPage ?>">
					...
					<div class="row">
						...
						<?php if (!empty($_resPagingHtml)) echo $_resPagingHtml; ?>
						</div>
					</div>
				</div><!-- -->
			</div>                
    ...
    <script>
		$(document).ready(function () {
				...
				**/
				...
				$('body').on('click', '.js-respage', function () {
					_isPageNav = true;
					$('#resPage').val($(this).data('page'));
					$('#base_code').submit();
				});
				$('#base_code').on('submit', function () {
					if (!_isPageNav) { $('#resPage').val(1); }
					_isPageNav = false;
				});
		})
		function chksave() {
