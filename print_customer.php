<?php
	...
	function custlist() {
			 ...
			 if (($prodInfo['p_code'] == 'LAPICKUP') || ($prodInfo['p_code'] == 'LVPICKUP') || ($prodInfo['p_code'] == 'PICKUP') || ($prodInfo['p_code'] == 'EWRPICKUP') || ($prodInfo['p_code'] == 'SFOPICKUP') || ($prodInfo['p_code'] == 'SLCPICKUP') || ($prodInfo['p_code'] == 'ANCPICKUP')  || ($prodInfo['p_code'] == 'SANPICKUP') || ($prodInfo['p_code'] == 'DFWPICKUP') || ($prodInfo['p_code'] == 'DFWPICKUP') || ($prodInfo['p_code'] == 'LGAPICKUP') || ($prodInfo['p_code'] == 'YULPICKUP') ) {
				...
				$order = "order by b.reserveCode asc";
			 }
			 
		     $qry1="select b.reserveCode,b.pricet,b.p_code,b.air_arcity,b.air_arrivetime,b.air_arriveNm,
			 b.air_stcity,b.air_sttime,b.air_stNm, b.p_name, b.reserveCode, b.book_pri, b.book_phone,b.rand_id,
			 b.room_cnt, b.p_cnt as pcnt, b.last_bal, b.stDate, b.base_rate, b.parent,b.userid 
			 from reserve_info b
			 where b.p_code='$s_code' && b.stDate ='$stdate' && b.rev_status ='DONE'  $order ";
				//		echo $qry1; 
			 ...
			 while($row1 = mysql_fetch_assoc($rst1)){
				...
				if (($prodInfo['p_code'] == $PICKUP['p_code']) && ($prodInfo['p_code']!="")) {
				    ...
				    $picnum = getPicGr6($row1['reserveCode']);
				///	echo "2222";
				} else {
				///	echo "3333";
				...
				$reTrinfo=getReserveTrRepre($row1['reserveCode']);
				...
				if ($row1['pricet'] == '3') {
					...
					echo "<tr>
							  <td rowspan='{$row1['cnt']}'><a href=javascript:openwin('{$row1['reserveCode']}','{$row1['pricet']}') >{$row1['book_pri']}</a></td>
							  <td ><a href=javascript:openwin('{$row1['reserveCode']}','{$row1['pricet']}') >{$reTrinfo['traveler_nm']}</a></td>
							  <td >{$row1['book_phone']}</td>
											...
											<table id="custom_table" class="table table-bordered table-condensed custom_table">
												<thead>
													<tr>  
													  ...
													  <th class="tcenter" width='7%'>대표고객</th>
