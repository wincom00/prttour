<?php
	...
	if ($mode == "sendemail") {
		$query = "SELECT reserveCode,p_name,p_code,book_pri,book_email,s_mail,parent
					FROM reserve_info
					WHERE tour_type != '3'
					  && (rev_status ='READY' || rev_status ='ORDER' || rev_status ='DONE')
					  && s_mail != 'y'
					  && reserveCode IN (
							SELECT reserveCode
							FROM tour_car
							WHERE sub_eCode='$s_code' && p_code='$p_code' && stDate='$stDate'
					  )
					  && parent='MAIN'";
		$rst1 = mysql_query($query);

		while ($row1 = mysql_fetch_assoc($rst1)) {
			$picnum = getPicGr4($row1['reserveCode'], $row1['book_pri']);
			...
			$sbj = str_replace('{nm1}', $row1['book_pri'], $sbj);
			$sbj = str_replace('{pm}', $refo['p_name'], $sbj);
			$sbj = strip_tags($sbj);
			$sbj = htmlspecialchars($sbj, ENT_QUOTES, 'UTF-8');

			$content = file_get_contents("http://www.myprt.online/ginfo.html", false, stream_context_create($arrContextOptions));
			$content = str_replace('{img2}', $prodInfo['p_img1'], $content);
			$content = str_replace('{nm}', $row1['book_pri'], $content);

			$date_parts = explode("-", $stDate);
			$date_string = $date_parts[0] . " 년" . $date_parts[1] . " 월" . $date_parts[2] . " 일";
			...
			mailsend_a('online@prttour.com', $sbj, $msg, $attachment1, $attachment2);
		}
	...
	function custlist() {
		...
		while ($row1 = mysql_fetch_assoc($rst1)) {
			...
			$rest = ($row1['last_bal'] == "0") ? "<font color=red>완납</font>" : "미납";
			...
			if ($row1['reserveCode'] != $orev) {
				$bgcolor = ($bgcolor == "style='background-color:#f3f3f3;'") ? "" : "style='background-color:#f3f3f3;'";
			}
			if ($row1['s_mail'] == 'y') {
				$bgcolor = "style='background-color:#3366ff !important; color:#fff;'";
			}
		...
		<div class="row">
			<div class="col-sm-12 col-md-12">
				<div class="col-sm-12" style='page-break-after:always'>
					<form action="" name="frmName" id="frmName" method="post">
						...
						<div class="row no-nav">
							<div id="custom_button" class="col-sm-12 text-right">
								<button type="button" class="btn btn-xs btn-default js-mail">가이드인폼이메일보내기</button>
							</div>
						</div>
						<?php endif; ?>
