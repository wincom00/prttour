	...
	function mailsend_a($to,$subj,$contents,$attachment1,$attachment2, $attachment3='', $attachment4='') {
			...
			try {
				...
				*/
				...
				$err = ($mail && isset($mail->ErrorInfo) && $mail->ErrorInfo) ? $mail->ErrorInfo : $e->getMessage();
				error_log('[mailsend_a] '.$err);
				return $err;
			} catch (Throwable $t) {
				$err = ($mail && isset($mail->ErrorInfo) && $mail->ErrorInfo) ? $mail->ErrorInfo : $t->getMessage();
				error_log('[mailsend_a] '.$err);
				return $err;
			}
	}
