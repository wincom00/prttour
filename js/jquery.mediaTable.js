...
;(function($){
	...
	var __loop = function( cfg, i ) {
		...
		var __initMenu = function( wdg ) {
			...
			wdg.$menu.$header 	= $('<a class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" href="#" />');
		...
		var __thInit = function( i, wdg ) {
			...
			if ( wdg.cfg.menu && !$th.is('.persist') ) {
				...
				var $li = $('<li><div class="checkbox"><label for="toggle-col-'+wdg.id+'-'+i+'"><input type="checkbox" name="toggle-cols" id="toggle-col-'+wdg.id+'-'+i+'" value="'+id+'" /> '+$th.text()+'</label></div></li>');
