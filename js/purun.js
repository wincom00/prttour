...
pt = Object.assign({}, pt || {}, {
	...
	initReservationDetail: function initReservationDetail() {
		...
		for (var i = 0; i < scope.length; i++) {
			...
			var hideShowToggleBtns1 = self.find('.js-hideShowToggle1')
			...
			hideShowToggleBtns1.off('click.showDatepicker').on('click.showDatepicker', function (e) {
				var btn = $(this)
				var chevron = btn.find('.glyphicon')
				var isShown = chevron.hasClass('glyphicon-chevron-down')
				var trToToggle = btn.closest('tr').siblings()
	...
	beforeShowDayFunc: function beforeShowDayFunc(date, set) {
		...
		var daysOfWeekEnabledSet = set.daysOfWeekEnabled && set.daysOfWeekEnabled.length >= 1 || false
		...
		if (set.st && set.st.length >= 1 && mdy < set.st) {
			inRange = false
		}
		if (set.et && set.et.length >= 1 && mdy > set.et) {
			inRange = false
		}

		if (datesOnlySet) {
			enabled = set.datesOnly.indexOf(mdy) !== -1
			isFixedDate = enabled
		} else if (inRange) {
			if (!daysOfWeekEnabledSet && !datesEnabledSet) {
				enabled = true
			}

			if (daysOfWeekEnabledSet) {
				var day = date.getDay()
				enabled = set.daysOfWeekEnabled.indexOf(day) !== -1
			}
		...
		if (datesDisabledSet && set.datesDisabled.indexOf(mdy) !== -1) {
			enabled = false
			isFixedDate = false
		}
		...
		return {
			enabled: true,
			classes: isFixedDate ? 'available-date available-date-fixed' : 'available-date'
		}
	},
	beforeShowDayFunc1: function beforeShowDayFunc1(date, set) {
		// if daysOfWeekDisabled is set, it will override anything set in this function
		var daysOfWeekEnabledSet = set.daysOfWeekEnabled && set.daysOfWeekEnabled.length <= 1 || true
		var datesEnabledSet = set.datesEnabled && set.datesEnabled.length >= 1 || true
		var datesDisabledSet = set.datesDisabled && set.datesDisabled.length >= 1 || false
		var datesOnlySet = set.datesOnly && set.datesOnly.length >= 1 || false
		var enabled
		var datest = set.st && set.st.length >= 1 || false
		//console.log(set.daysOfWeekEnabled.indexOf(day)+'qqqq');
		//if (daysOfWeekEnabledSet || datesEnabledSet ) {
			if (datesOnlySet) {
				var mdy = moment(date).format('YYYY-MM-DD')
				if (set.datesEnabledSet.indexOf(mdy) !== -1) {
					enabled = true
				} else {
					enabled = false
				}
				//alert(datesOnlySet);
				console.log(set.et+'ssss');
				var mdy = moment(date).format('YYYY-MM-DD');
				...
				var mdy = moment(date).format('YYYY-MM-DD')
					console.log(set.et+'eeee');
				if (mdy >= set.st && mdy <= set.et) {
				
					if ((daysOfWeekEnabledSet) || (datesEnabledSet)) {
						var day = date.getDay()
						///console.log(day);
						//console.log(set.daysOfWeekEnabled.indexOf(day));
						if ((set.daysOfWeekEnabled.indexOf(day) !== -1) || (set.datesEnabled.indexOf(mdy) !== -1)) {
							enabled = true
						} else {
							enabled = false
						}
				...
				if (datesDisabledSet) {
					...
					if (datesDisabledSet) {
						//console.log( mdy+'tttt'+set.datesEnabled.indexOf(mdy));
						set.datesDisabled = this.removeSameItem(set.datesDisabled,set.datesEnabled)
					}
					if (set.datesDisabled.indexOf(mdy) !== -1) {
						enabled = false
					}
				}
		...
		//}
	    //console.log(datesOnlySet+"++"+enabled+"++"+mdy);
		return enabled
	},
	...
	changeTourStartDate: function changeTourStartDate(e) {
		...
		$.getJSON("get_pgrev.php?pcode="+pcode+"&st="+stDate3, function(jsonData){
		...
		$.getJSON("get_rev.php?pcode="+pcode+"&st="+stDate3, function(jsonData){
