<?php
	...
	$gscode = isset($_REQUEST['gscode']) ? $_REQUEST['gscode'] : (isset($gscode) ? $gscode : '');
	$pcode  = isset($_REQUEST['pcode']) ? $_REQUEST['pcode'] : (isset($_REQUEST['p_code']) ? $_REQUEST['p_code'] : (isset($pcode) ? $pcode : ''));
	$st     = isset($_REQUEST['st']) ? $_REQUEST['st'] : (isset($_REQUEST['stdate']) ? $_REQUEST['stdate'] : (isset($_REQUEST['stDate']) ? $_REQUEST['stDate'] : (isset($st) ? $st : '')));
...
<body>
    ...
    <script>
		...
		var ctr=0;
        function openwin(g_code,s_code,p_code,st) { 
	       var winName = "all_"+(ctr++);
		   window.open("guide_assign_customer6.php?g_code="+g_code+"&s_code="+s_code+"&p_code="+p_code+"&st="+st,"customer","width=1050,height=700,scrollbars=1");
	    }
