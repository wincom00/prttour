    ...
    function file_save($the_file, $directory) {
		...
		$msg1 = "";
			...
			if ($the_file["error"] > 0) {
				...
				// $_dbg = (is_writable(__DIR__ . '/..') ? __DIR__ . '/..' : sys_get_temp_dir()) . '/file_save_debug.log';
				// $_dlog = function($line) use ($_dbg) {
				// 	$entry = date('[Y-m-d H:i:s] ') . $line . "\n";
				// 	file_put_contents($_dbg, $entry, FILE_APPEND);
				// 	error_log('[file_save] ' . $line);
				// };

				// // ▼▼▼ DEBUG ▼▼▼
				// $_dlog("STEP1 directory   : {$directory}");
				// $_dlog("STEP2 full_path    : {$directory}{$file_name}");
				// $_dlog("STEP3 dir_exists   : " . (is_dir($directory)      ? 'Y' : 'N'));
				// $_dlog("STEP4 writable     : " . (is_writable($directory) ? 'Y' : 'N'));
				// $_dlog("STEP5 is_uploaded  : " . (is_uploaded_file($the_file['tmp_name']) ? 'Y' : 'N'));
				// // ▲▲▲ DEBUG ▲▲▲

				if (is_uploaded_file($the_file['tmp_name'])) {

					$move_ok = move_uploaded_file($the_file["tmp_name"], $directory . $file_name);

					// // ▼▼▼ DEBUG ▼▼▼
					// $_dlog("STEP6 move_result  : " . ($move_ok ? 'OK' : 'FAIL'));
					// $_dlog("STEP6 dest         : {$directory}{$file_name}");
					// if (!$move_ok) $_dlog("STEP6 php_errstr   : " . (error_get_last()['message'] ?? 'unknown'));
					// // ▲▲▲ DEBUG ▲▲▲

					$_rf = remote_detect_folder($directory);

					// // ▼▼▼ DEBUG ▼▼▼
					// $_rf_ref = new ReflectionFunction('remote_detect_folder');
					// $_dlog("STEP7 fn_file       : " . $_rf_ref->getFileName() . " L" . $_rf_ref->getStartLine());
					// $_dlog("STEP7 detect_folder : " . ($_rf ?? 'null (FTP 스킵)'));
					// // ▲▲▲ DEBUG ▲▲▲

					if ($_rf) {
						$ftp_err = '';
						$ftp_ok  = remote_sync_file("{$directory}{$file_name}", $_rf, $ftp_err);
						// // ▼▼▼ DEBUG ▼▼▼
						// $_dlog("STEP8 ftp_result   : " . ($ftp_ok ? 'OK' : 'FAIL'));
						// if (!$ftp_ok) $_dlog("STEP8 ftp_error    : {$ftp_err}");
						// // ▲▲▲ DEBUG ▲▲▲
					}
...
//echo $code1 ."|".$selectValueInput;
  ...
  function getCruiseinfo($estimateCode){
	...
	function getCruiseinfoList($estimateCode){
		
		global $dbConn;

		$qry1 = "select * from reserve_cruise where reserveCode = '$estimateCode' order by c_seqm asc, seq_no asc";
		$rst1 = mysql_query($qry1);
		$rows = array();
  ...
  function getinfo_menufst($id,$divi){
   ...
   function printMenu($id){
    ...
    function printBaseCode9($lvcode1,$lvcode2,$code = false){
		...
		function printBaseCode4($lvcode1, $code = false){
