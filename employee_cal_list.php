...
*/
    ...
    $searchMethod  = isset($_REQUEST['searchMethod'])  ? trim($_REQUEST['searchMethod'])  : '';
    ...
    function printPay(&$totalCount = 0)
        ...
        if (!empty($searchMethod)) {
            $sm = mysql_real_escape_string($searchMethod);
            $where[] = "b.pay_method = '$sm'";
        }
        ...
        while($row1 = mysql_fetch_assoc($rst1)){
            ...
            if ($isAutoConfirmed) {
                ...
                $confText = "<button type='button' class='btn btn-xs btn-primary js-confirmOne' data-id='{$row1['seq_no']}'>확인</button>";
...
<body>
                                ...
                                <tr>
                                    ...
                                    <td colspan="2" class="text-center formHeader">
                                        <select class="form-control" name="searchMethod">
                                            ...
                                            <option <?php if ($searchMethod == "cash") { ?> selected <?php } ?> value="cash">현금</option>
                                            <option <?php if ($searchMethod == "creditcard") { ?> selected <?php } ?> value="creditcard">신용카드웹</option>
                                            <option <?php if ($searchMethod == "debitcard") { ?> selected <?php } ?> value="debitcard">데빗</option>
                                            <option <?php if ($searchMethod == "bcreditcard") { ?> selected <?php } ?> value="bcreditcard">신용카드 자사단말기</option>
                                            <option <?php if ($searchMethod == "check") { ?> selected <?php } ?> value="check">체크</option>
                                            <option <?php if ($searchMethod == "banktransfer") { ?> selected <?php } ?> value="banktransfer">은행송금</option>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.js-confirmOne', function(){
                var $btn = $(this);
                var seq = $btn.data('id');
                if (!seq) return;
                if (!confirm('확인처리하시겠습니까?')) return;
                ...
                $.ajax({
                    ...
                    if (res && res.ok) {
                        ...
                        $btn.replaceWith("<span class='accspan text-primary'>확인완료</span>");
                        $tr.find('td:last').attr('data-sort-value', '2');
                        $tr.find('.js-rowCheck').prop('checked', false).prop('disabled', true);
                    } else {
                        alert(res && res.msg ? res.msg : '처리 중 오류가 발생했습니다.');
